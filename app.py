# app.py — Gymapp (simplified & stable)
# Antaganden (matchar ditt schema):
# - program_weeks: week, day_label, exercise_id (UUID), sets, rep_min, rep_max
# - workouts: id (UUID), date, day_label
# - sets: (workout_id UUID, exercise_id UUID, set_no, reps, weight_kg, pr_flag)
# Secrets hämtas från:
# - st.secrets (SUPABASE_URL/SUPABASE_KEY eller [supabase]{url, anon_key})
# - env
# - ./streamlit_config/secrets.toml
# - /Users/tobias/Documents/Gymapp/streamlit_config/secrets.toml
...
def is_lower_body(name: str) -> bool:
    return any(w in name.lower() for w in ["böj","squat","mark","rdl","hip","thrust","vad","lung","front","goblet","pull-through"])

def weight_round(w: float) -> float:
    return round(w * 2) / 2 if w is not None else 0.0

def best_previous_weight(ex_id: str) -> float:
    rows = sb.from_("sets").select("weight_kg").eq("exercise_id", ex_id).order("weight_kg", desc=True).limit(1).execute().data or []
...
def best_previous_reps_at_weight(ex_id: str, weight: float) -> int:
    rows = sb.from_("sets").select("reps").eq("exercise_id", ex_id).eq("weight_kg", weight).order("reps", desc=True).limit(1).execute().data or []
...
def last_two_sessions_under_min(ex_id: str, rep_min: int) -> bool:
    rows = sb.from_("sets").select("workout_id, reps").eq("exercise_id", ex_id).order("workout_id", desc=True).limit(400).execute().data or []
...
def propose_weight(ex_name: str, ex_id: str, rep_min: int, rep_max: int) -> float:
    rows = sb.from_("sets").select("weight_kg").eq("exercise_id", ex_id).order("workout_id", desc=True).limit(12).execute().data or []
    base = (40.0 if is_lower_body(ex_name) else 20.0) if not rows else float(pd.DataFrame(rows)["weight_kg"].dropna().max() or (40.0 if is_lower_body(ex_name) else 20.0))
    if last_two_sessions_under_min(ex_id, rep_min):
        return weight_round(base * 0.95)
    last_wid = sb.from_("sets").select("workout_id").eq("exercise_id", ex_id).order("workout_id", desc=True).limit(1).execute().data or []
    if not last_wid: return weight_round(base)
    wid = last_wid[0]["workout_id"]
    reps = sb.from_("sets").select("reps").eq("exercise_id", ex_id).eq("workout_id", wid).order("set_no", asc=True).execute().data or []
    if reps and all(int(r["reps"] or 0) >= rep_max for r in reps):
...
def create_workout(d: date, day_value: str) -> str:
    wid = str(uuid4())  # generera UUID här → ingen RETURNING behövs
...
def upsert_set(wid: str, ex_id: str, set_no: int, reps: int, weight_kg: float, pr_flag: bool):
    ...
    exists = sb.from_("sets").select("workout_id").eq("workout_id", wid).eq("exercise_id", ex_id).eq("set_no", set_no).execute().data or []
    if exists:
        sb.from_("sets").update(payload).eq("workout_id", wid).eq("exercise_id", ex_id).eq("set_no", set_no).execute()
    else:
...
def is_pr(ex_id: str, reps: int, weight_kg: float) -> bool:
    ...
    if weight_kg == prev_best_w: return reps > best_previous_reps_at_weight(ex_id, weight_kg)
    return False

# ---------------- Header ----------------
today = date.today()
widx = get_week_index(today)
st.markdown(f"### 💪 Gymapp  &nbsp;&nbsp; **{today.strftime('%Y-%m-%d')}**  &nbsp;&nbsp; **Vecka {widx+1}** <span class='badge'>{phase_for_week(widx)}</span>  &nbsp;&nbsp; <span class='pill pill-live'>🟢 LIVE</span>", unsafe_allow_html=True)
        ...
        with st.form("today_form"):
            ...
            for row in plan:
                ...
                for s_no in range(1, sets_n + 1):
                    ...
                    with c3:
                        st.markdown(f"<div class='muted'>Mål: {rep_min}-{rep_max}</div>", unsafe_allow_html=True)
                st.divider()
...
def seed_program() -> int:
    ...
    plan = {
        ...
        "Upper B": [
            ("Hantelpress plan bänk", True, 4),
            ("Kabel-flyes (låg→hög)", False, 3),
            ("Lutande kabelpress", False, 3),
            ("Sittande kabelrodd", True, 4),
            ("Face pull", False, 3),
        ...
        "Lower B": [
            ("Marklyft", True, 3),
            ("Frontböj/Goblet squat", True, 3),  # exakt som din DB
            ("Hip thrust", True, 4),
            ("Bakåtlunges", False, 3),
            ("Vadpress", False, 3),
    ...
    for week in range(1, 13):
        for day_name, ex_list in plan.items():
            for (name, is_base, base_sets) in ex_list:
                ex_id = name_to_id.get(name)
...
with tabs[2]:
    ...
    if not ws:
        ...
        for w in ws:
            ...
            if not rows:
                st.caption("Inga set."); continue
            df = pd.DataFrame(rows)
            df["övning"] = df["exercises"].apply(lambda x: x["name"] if isinstance(x, dict) else "")
            df = df[["övning","set_no","weight_kg","reps","pr_flag"]].rename(columns={"set_no":"set","weight_kg":"kg","reps":"reps","pr_flag":"PR"})
    ...
    with st.form("export_form"):
        ...
        if go:
            data = (
                ...
                .order("workout_id", asc=True).order("exercise_id", asc=True).order("set_no", asc=True)
                .execute().data or []
            )
            if not data:
